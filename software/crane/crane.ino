...
void buttonHandler(){
  ...
  //an example of multiple button pressed (in this case when button 2 and 3 are pressed the backlight toggles)
    ...
    if(buttonEdge[11] == 0 ){
      ...
      tft.setTextSize(2);
      tft.setCursor(0, 0);
      tft.println("Hello World! How are you doing? This is the controller board for the Mizzan microtonal piano project. ");
...
void loop(void) {
  ...
  delay(1000);
  */
  //Serial.println("HOLLA HOLLA");
  //int btn = digitalRead(CRANE_KEY_R0);
  //digitalWrite(TFT_BL,btn );
  ...
  for(i = 0;i<12;i++){
    buttonID = i;
    if(buttonFlag[buttonID] == 1){
      if(buttonEdge[buttonID] == 1){
        sprintf (textBuff, "button %d: RISE\r\n", buttonID);
        Serial.print(textBuff);
      }else{
        sprintf (textBuff, "button %d: FALL\r\n", buttonID);
        Serial.print(textBuff);
      }
      buttonFlag[buttonID] = 0; //clear the flag
    }
  }
  */
